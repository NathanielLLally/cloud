...
# get a default project from the list of projects
# 
open(CMD, "gcloud projects list|") || die "no gcloud";
while (<CMD>) {
    if ($_ =~ /(GoogleAuthWeb\-\d+)/) {
        $project=$1;
    };
  print $_ if (defined $DEBUG);
}
$project = 'ethereal-smoke-313104';
...
if ((not defined $name) or (not defined $zone)) {
  print "there are zero provisioned instances\n";
} else {
